...
A session that was mid-task when it was refreshed is nudged to pick the work back up: the resumed session gets a short `continue where you left off` prompt after the reload commands, so killing a working session doesn't quietly abandon whatever it was doing. Whether it was working is read from the pane right before the kill, matched against the parenthetical on Claude Code's spinner line (`(12s · ↑ 1.4k tokens · esc to interrupt)`), which is only drawn while a turn is actually running — so a session sitting idle, one whose transcript merely quotes that phrase, and one waiting at a permission prompt are all left alone. The prompt also needs a session to continue *into*: a refresh that recovered no session ID relaunches a brand-new `claude` with no prior transcript, and is never sent the prompt.
