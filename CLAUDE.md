...
Cross-session address reporting is the second entry, and the mechanism behind the card copy button (see `README.md`). A session's `SendMessage` address exists only inside the Claude Code harness, so `bin/hooks/session-address.sh` has the session report its own: `--announce` (`SessionStart`) asks the model to read its name out of `ListAgents` once and POST it, `--heartbeat` (`UserPromptSubmit`) is a shell-only liveness ping that re-asks only when the dashboard says it has no address, and `--end` (`SessionEnd`) unregisters. It rides `UserPromptSubmit` rather than `Stop` because that is where `additionalContext` is honored, which is what lets a session recover its registration after a dashboard restart without the user doing anything. Correlation is by **tty**, not directory — two panes in one repo share a directory but never a tty — resolved by `bin/hooks/lib/resolve-pane-tty.sh` (shared with the worktree hook; its `command=` vs `comm=` truncation comment is load-bearing). `src/session-addresses.js` keeps the registry in memory only, pruned three ways: TTL, reconciliation against live cmux ttys each poll, and explicit unregister — an address that outlived its session is worse than none, since the card would offer something that silently goes nowhere.
...
- `src/session-addresses.js` - In-memory registry of self-reported cross-session `SendMessage` addresses, keyed by tty
