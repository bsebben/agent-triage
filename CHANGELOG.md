...
- The dashboard's own host workspace now gets a real card in the workspace list (title and focus-to-select only — no dismiss/close/refresh/rename/new-session, since those don't make sense for the terminal running this server, and closing it would kill the server). Previously the host was excluded entirely, so cmux's native focus landing on its tab left every card in the dashboard unhighlighted with no way to tell where you were. The host gets its own dedicated group that always sorts last, regardless of its title — separate from its actual directory's group, so real agent activity sharing that same repo (e.g. someone working on this dashboard's own code) keeps its normal alphabetical spot instead of being pulled down with the host.
...
- `Monitor.poll()` had no reentrancy guard, so an event-triggered poll could overlap with the regular 5s-timer poll — including issuing concurrent `reorder-workspaces` calls against the same cmux window, which corrupted cmux's own tab/workspace state during testing (a second window appeared with duplicate-looking workspaces under fresh ids). Overlapping calls now defer and run once, right after the in-flight one finishes.
- `notification.list` is global, but `workspace.list` is scoped to Agent Triage's own window — a notification for a workspace outside that window (e.g. from a stray second cmux window) used to linger forever as an unresolvable "Unknown" card, since it could never be matched to a title/directory. Notifications are now scoped to known workspaces, and stale ones are reaped like any other removed workspace.
- `Monitor`'s `subscribeWorkspaceEventsFn` fallback bypassed an injected `cmuxApi`'s own `subscribeWorkspaceEvents`, going straight to the real cmux module instead — fixed to check the resolved `cmuxApi` first.
- The dashboard's Cmd+↑/↓ handler only excluded `<input>` fields, not `<textarea>` or `contenteditable` elements, so it hijacked the native macOS text-navigation shortcut there.
- The workspace event-stream socket silently swallowed connection errors with no logging, so a permanently failed connection (instant-update stops working, falls back to 5s polling) had zero visibility.
...
- Question pills: workspace cards no longer scrape terminal screen content to guess at multi-choice menus and render clickable answer buttons. The heuristic had no structured signal to work from (it can't tell an AskUserQuestion menu from a permission prompt or an arbitrary CLI confirm), so it was fundamentally unable to be both complete and accurate — every fix for a false positive introduced new false negatives. Cards for "waiting" notifications now just show the plain notification body, same as before question detection kicked in. Also removes the now-unused `/api/respond` endpoint and `cmux.readScreen`.
...
- The merged-PR search uses a `sort:updated-desc` qualifier to skew GitHub's first-100-result window toward the most recent merges. Known limitation: no pagination past 100 results, so an author with more than 100 merges in 30 days may not see every repo's most-recent merges.
